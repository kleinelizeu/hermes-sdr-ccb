...
Antes isso acontecia quando a conexão do túnel (Cloudflare) caía sozinha depois de alguns dias e o serviço ficava parado até reiniciar na mão. **Agora há um vigia automático** (`hermes-sdr-watchdog`) que verifica a cada minuto, **religa o túnel sozinho**, registra tudo em `/var/log/hermes-sdr-webhook.log` e, se o endereço mudar, **atualiza o Zernio sozinho pela API** — você não precisa fazer nada. Só se a API do Zernio estiver fora é que ele te avisa no Telegram para colar o endereço novo na mão (*Webhooks → seu webhook → Endpoint URL*). Para ver o histórico: `tail -f /var/log/hermes-sdr-webhook.log`. Se mesmo assim algo persistir, rode `hermes-sdr doctor`.
