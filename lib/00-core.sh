#!/usr/bin/env bash
# 00-core.sh — Fundação do wizard Hermes SDR by CCB
# Banner, UI, perguntas, validadores, estado e idempotência.
# Carregado por instalar.sh e doctor.sh (não executa nada sozinho).

# ── Estado / diretórios ───────────────────────────────────────────────────────
ESTADO_DIR="${ESTADO_DIR:-/root/.hermes-sdr-ccb}"
ESTADO_CONFIG="$ESTADO_DIR/config"
ESTADO_PASSOS="$ESTADO_DIR/passos"
ESTADO_CONTEXTO="$ESTADO_DIR/business-context.md"
ESTADO_RESPOSTAS="$ESTADO_DIR/respostas-negocio"
...
executar_passo() {
  ...
  if "$fn"; then
    ...
    erro "Falhou em: $desc"
    dica "Você pode rodar de novo com 'hermes-sdr' que ele continua daqui."
    exit 1
  fi
}
