...
_instalar_cloudflared() {
  ...
  if curl -fsSL -o "$deb" \
    ...
    dica "Instale manualmente (https://pkg.cloudflare.com) e rode 'hermes-sdr' de novo."
    return 1
  fi
}
...
_capturar_url_tunel() {
  ...
  if [[ -z "$url" ]]; then
    dica "O túnel subiu mas ainda não mostrou o endereço. Rode 'hermes-sdr doctor' em 1 minuto."
    return 0
  fi
  ...
  caixa "IMPORTANTE: este endereço pode mudar se a VPS reiniciar." \
        "Se as respostas pararem, rode:  hermes-sdr doctor" \
